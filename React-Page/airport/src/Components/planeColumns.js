...
function PredictButton(C,R,ok,rm,type){
    if(ok===undefined){
        if(JSON.parse(sessionStorage.tickets).find(x=>x.scolum==C && x.srow==R && x.isreturn==type)){
            if (type=="0"){
                return <Button variant="primary">{R}-{C}</Button>
            }else{
                return <Button  variant="secondary">{R}-{C}</Button>
            }
        ...
        return(<Button variant="outline-success">{R}-{C}</Button>);
